...
{
    return (Degree >= 0) ? (((Degree == 0) || (Degree > 3)) ? (std::pow(value, Type(1)/Type(Degree))) : ((Degree == 3) ? (std::cbrt(value)) : ((Degree == 2) ? (std::sqrt(value)) : (value)))) : (Type(1)/rt<(-Degree)*(Degree < 0), Type>(value));
}
